...
namespace Drupal\custom_1\Controller;
 ...
 */
class custom_1Controller {

  /**
   * Constructs Lorem ipsum text with arguments.
   * This callback is mapped to the path
   * 'custom_1/generate/{paragraphs}/{phrases}'.
   * 
  ...
  public function generate($paragraphs, $phrases) {
    // Default settings.
    $config = \Drupal::config('custom_1.settings');
    // Page title and source text.
    $page_title = $config->get('custom_1.page_title');
    $source_text = $config->get('custom_1.source_text');
